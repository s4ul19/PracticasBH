...
y por tanto es válido asumir la universalidad de RMT para el análisis\janote{no modificaste este enunciado. Toca quitar la primera parte, porque discutimos ayer que no es cierto}.
Estudiando la distribución de probabilidad asociada a las $r$'s\janote{siempre que pongás variables matemáticas, asegurate que las pongás dentro de \$\$!! Borrá este comentario cuadno lo veás}, es posible determinar si la configuración está en un regímen integrable 
...
regímen\janote{este enunciado está mal redactado, volvelo a escribir. Empezás hablando de la PDF y despúes de la coma saltás a  hablar de la $\ev{r}$, son dos ideas que no estás conectando adecuadamente. Asegurate que se tu modificación haga una transición suave con el siguiente enunciado (o modficá acorde el enunciado siguiente también)}. En particular, para el ensamble GOE, dicho valor es de $\ev{r}_{GOE}=0.5307$~\cite{Atas_2013} y para un regímen integrable\janote{la distribución de Poisson*} un valor de $\ev{r}_{Poisson}=0.38629$~\cite{Atas_2013}. 
...
\janote{mirá cómo hice para que los paréntesis se vean bien. Cambialo así en la siguiente ecuación}
...
\begin{itemize}
    \item Tarea 1: Estudiar sobre de las simetrías en un Hamiltoniano y su sectorización\janote{su forma diagonal por bloques debido a la simetría*}.
\janote{Agregá una tarea de estudiar sobre el modelo de Bose-Hubbard}
    ...
    \item Tarea 9: Presentar los datos obtenidos de tal forma que sea clara la exposción de información de diagnostico.\janote{Mejor cambiá esto por escribir el informe final}
\end{itemize}
\input{cronograma}
